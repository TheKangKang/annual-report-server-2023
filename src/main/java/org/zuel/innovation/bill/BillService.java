...
//    }
    ...
    public BaseResult login(String account, int type, int gender, int identity) {
        Session session = sessionService.add(account, type, gender,identity);
        Tool data = new Tool();
        if (StrKit.notNull(session)) {
            //token值
            data.setAccessToken(session.getSessionId());
            // session表的自增id
            data.setSessionId(BigInteger.valueOf(session.getId()));
